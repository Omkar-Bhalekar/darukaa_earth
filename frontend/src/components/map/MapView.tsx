...
export function MapView({
  ...
  useEffect(() => {
    ...
    instance.on('load', () => {
      ...
      instance.addLayer({
        ...
        source: 'sites',
        paint: {
          'fill-color': ['coalesce', ['get', 'color'], '#52796F'],
          'fill-opacity': 0.5,
        },
      });
      instance.addLayer({
        ...
        type: 'line',
        source: 'sites',
        paint: { 'line-color': '#FEFAE0', 'line-width': 2 },
      });
  ...
  useEffect(() => {
    ...
    if (!instance || !instance.isStyleLoaded() || !instance.getSource('sites'))
      return;
    (instance.getSource('sites') as mapboxgl.GeoJSONSource).setData(
      geojson ?? { type: 'FeatureCollection', features: [] },
    );
  }, [geojson]);
